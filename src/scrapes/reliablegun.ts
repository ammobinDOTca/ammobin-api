...
async function makeReliableRequest(ammotype, page = 1) {
  ...
  return helpers.makeWrapApiReq('reliablegun', ammotype, page).then(d => {
    console.log(
      `reliablegun:  loaded ${ammotype} page${d.page} of ${d.lastPage}`
    )
    if (!isNaN(d.lastPage) && d.page < d.lastPage) {
      return makeReliableRequest(ammotype, page + 1).then(dd =>
        d.items.concat(dd)
      )
    } else {
      return d.items
...
export function reliablegun(type) {
  if (type === 'rimfire') {
    return makeReliableRequest('rimfire-ammunition').then(
    ...
    return makeReliableRequest('shotgun-ammunition').then(
