...
function makeWolverine(type) {
  ...
  function isReloadingComponent(item): boolean {
    return (
      item.Attributes.filter((a) => a.AttributeName === 'SUBCATEGORY' && a.AttributeValue === 'RELOADING COMPONENTS')
        .length > 0
    )
  }
...
export function wolverinesupplies(type: ItemType) {
  ...
  switch (type) {
    ...
    case ItemType.centerfire:
      return Promise.all(['rifle', 'pistol'].map((t) => throttle(() => makeWolverine(t)))).then(helpers.combineResults)
