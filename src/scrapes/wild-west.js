...
function fn(type, page = 1) {
  return axios.get(`https://gun-shop.ca/product-category/ammunition/${type}/page/${page}/?orderby=price-desc`)
    .then(r => {
      ...
      $('.product').each((index, row) => {
        ...
        result.price = parseFloat(tha.find('.amount').last().text().replace('$', ''));
