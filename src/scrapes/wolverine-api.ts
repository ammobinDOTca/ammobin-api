...
export function wolverinesupplies(type) {
  const throttle = throat(1)

  switch (type) {
    case 'rimfire':
      return makeWolverine('rimfire').then(helpers.classifyRimfire)
    ...
    case 'shotgun':
      return makeWolverine('shotgun').then(helpers.classifyShotgun)
