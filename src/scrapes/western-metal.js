  ...
  return axios.get(`https://www.westernmetal.ca/shooting-category/${section}?field_product_types_tid[]=${type}&sort_order=DESC&page=${page}`)
    .then(r => {
      ...
      $('.view-content .views-row').each((index, row) => {
        ...
        result.link = 'https://www.westernmetal.ca' + tha.find('.views-field-title a').prop('href');
        result.img = tha.find('.views-field-field-product-image img').prop('src');
        const split = result.link.split('/');
        result.name = split[split.length - 1].split('-').join(' ')
        result.price = parseFloat(tha.find('.views-field-php').text().replace('$', ''));
